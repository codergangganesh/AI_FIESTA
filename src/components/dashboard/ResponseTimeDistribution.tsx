...
export default function ResponseTimeDistribution({ 
  ...
  useEffect(() => {
    if (data.length > 0) {
      ...
      // Show all items when "View All" is clicked, otherwise show first 8
      setDisplayedData(showAll ? data : data.slice(0, 8))
    }
  }, [data, showAll])
  ...
  return (
    <div className={`rounded-2xl p-6 transition-colors duration-200 ${
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {data.length > 8 && ( // Show View All button when there are more than 8 items
          <button
